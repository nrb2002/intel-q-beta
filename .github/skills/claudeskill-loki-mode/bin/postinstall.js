...
const version = (() => {
  try {
    return fs.readFileSync(path.join(packageDir, "VERSION"), "utf8").trim();
  } catch {
...
try {
  ...
  const npmBin = execSync("npm bin -g 2>/dev/null || npm prefix -g", {
    encoding: "utf8",
  ...
  if (os.platform() === "darwin") {
    ...
    try {
      stableDir =
        execSync("npm prefix -g", { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] }).trim() +
        "/bin";
    } catch {
...
try {
  ...
  if (missing.length > 0) {
    ...
    try {
      execSync(`python3 -m pip install --break-system-packages ${missing.join(" ")}`, {
        stdio: "pipe",
        timeout: 60000,
      });
      ...
      try {
        ...
        console.log(
          `  [WARN] Could not install Python deps. Run: pip install ${missing.join(" ")}`,
        );
      }
    }
  }
