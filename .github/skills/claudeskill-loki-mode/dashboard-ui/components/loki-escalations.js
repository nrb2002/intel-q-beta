...
export class LokiEscalations extends LokiElement {
  ...
  render() {
    ...
    if (this._loading && this._items.length === 0) {
      ...
      body =
        '<div class="esc-error">Failed to load escalations: ' +
        this._escapeHtml(this._error) +
        "</div>";
    } else if (!this._items || this._items.length === 0) {
      body =
        '<div class="esc-empty">Escalations: no events yet. Handoff/escalation markdown documents written by the runner under .loki/escalations/ will appear here.</div>';
    } else {
      const rows = this._items
        .map((it) => {
          ...
          return (
            '<div class="esc-item" data-filename="' +
            fname +
            '">' +
            '<span class="esc-name">' +
            fname +
    ...
    if (this._activeFile) {
      ...
      if (this._activeBodyError) {
        inner =
          '<div class="esc-error">Failed to load: ' +
          this._escapeHtml(this._activeBodyError) +
          "</div>";
