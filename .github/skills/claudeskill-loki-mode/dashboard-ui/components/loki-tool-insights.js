...
export class LokiToolInsights extends LokiElement {
  ...
  _render() {
    ...
    for (const tool of tools) {
      ...
      const totalLen =
        (tool.description || "").length + JSON.stringify(tool.input_schema || {}).length;
      if (totalLen > 10000) {
        warnings.push(
          `${tool.name}: description exceeds 10K chars (${totalLen} chars). May impact performance.`,
        );
      }
    }
    ...
    // Build origin rows
    let originRows = "";
    for (const [origin, group] of Object.entries(grouped).sort(
    ...
    if (Object.keys(toolMetrics).length > 0) {
      const sorted = Object.entries(toolMetrics).sort(
        (a, b) => (b[1].count || b[1]) - (a[1].count || a[1]),
      );
      metricsHtml = `
        <div class="metrics-section">
          ...
          ${sorted
            ...
            .map(([name, data]) => {
              ...
              const success =
                typeof data === "object" ? data.success_rate || data.success || 100 : 100;
              return `
