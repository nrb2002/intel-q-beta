...
export class LokiKpiCard extends LokiElement {
  ...
  _renderSparkline(dataStr) {
    ...
    const fillColor =
      trend === "up"
        ? "var(--loki-green-muted)"
        : trend === "down"
          ? "var(--loki-red-muted)"
          : "var(--loki-bg-tertiary)";
  ...
  _getTrendArrow(trend) {
    if (trend === "up")
      return '<svg class="trend-arrow up" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="18 15 12 9 6 15"/></svg>';
    if (trend === "down")
      return '<svg class="trend-arrow down" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg>';
    return "";
  }
  ...
  render() {
    ...
    const displayValue = isNaN(parseFloat(rawValue))
      ? rawValue
      : this._formatValue(this._animatedValue || parseFloat(rawValue));
