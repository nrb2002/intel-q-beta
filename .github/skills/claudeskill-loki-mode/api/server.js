...
async function handleChat(body) {
  ...
  if (processManager.status === "running") {
    try {
      ...
      const assistantMsg = {
        ...
        role: "assistant",
        content:
          "Message sent to Loki Mode session. The AI will process it in the current workflow.",
        timestamp: new Date().toISOString(),
        provider,
      };
  ...
  const assistantMsg = {
    ...
    role: "assistant",
    content:
      'Loki Mode is not currently running. Start a session first with the "Start" button, then your messages will be processed by the AI.',
    timestamp: new Date().toISOString(),
    provider,
  };
  chatHistory.push(assistantMsg);
...
class ProcessManager {
  ...
  async start(options = {}) {
    ...
    if (prd) {
      ...
      if (
        !resolvedPrd.startsWith(resolvedProjectDir + path.sep) &&
        resolvedPrd !== resolvedProjectDir
      ) {
  ...
  parseLogLine(line) {
    ...
    const level = clean.includes("[ERROR]")
      ? "error"
      : clean.includes("[WARN]")
        ? "warn"
  ...
  async injectInput(input) {
    ...
    if (!PROMPT_INJECTION_ENABLED) {
      throw new Error(
        "Prompt injection is disabled for security. Set LOKI_PROMPT_INJECTION=true to enable (only in trusted environments).",
      );
    }
