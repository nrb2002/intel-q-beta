...
async function startServer() {
  ...
  fs.writeFileSync(
    path.join(TEST_DIR, ".loki", "dashboard-state.json"),
    JSON.stringify({
      phase: "DISCOVERY",
      mode: "idle",
      iteration: 0,
...
async function testPauseResume() {
  ...
  if (pauseRes.status === 200 && pauseRes.data.status === "paused") {
    ...
    fail(
      "Pause endpoint",
      `Expected 200 with paused status, got ${pauseRes.status}: ${JSON.stringify(pauseRes.data)}`,
    );
  }
  ...
  if (resumeRes.status === 200 && resumeRes.data.status === "running") {
    ...
    fail(
      "Resume endpoint",
      `Expected 200 with running status, got ${resumeRes.status}: ${JSON.stringify(resumeRes.data)}`,
    );
  }
...
async function testInvalidProvider() {
  ...
  if (res.status === 500 && res.data.error && res.data.error.includes("Invalid provider")) {
    ...
    fail(
      ...
      `Expected 500 with Invalid provider error, got ${res.status}: ${JSON.stringify(res.data)}`,
    );
  }
}
...
async function testPathTraversal() {
  ...
  if (res.status === 500 && res.data.error && res.data.error.includes("path traversal")) {
    ...
    fail(
      ...
      `Expected 500 with path traversal error, got ${res.status}: ${JSON.stringify(res.data)}`,
    );
  }
}
