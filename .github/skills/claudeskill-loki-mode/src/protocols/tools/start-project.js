...
const schema = {
  name: TOOL_NAME,
  description:
    "Start a new Loki Mode project from a PRD. Accepts PRD content directly or a file path.",
  inputSchema: {
    type: "object",
    properties: {
      prd: {
...
function execute(params) {
  ...
  if (fs.existsSync(sessionPath)) {
    try {
      ...
      /* corrupted session file, safe to overwrite */
    }
  }
  ...
  try {
    fs.writeFileSync(path.join(stateDir, "session.json"), JSON.stringify(session, null, 2), "utf8");
  } catch (err) {
    return {
      success: false,
