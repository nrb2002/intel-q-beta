...
class PluginValidator {
  ...
  validate(pluginConfig) {
    ...
    if (!VALID_PLUGIN_TYPES.includes(pluginConfig.type)) {
      errors.push(
        `Unknown plugin type: "${pluginConfig.type}". Valid types: ${VALID_PLUGIN_TYPES.join(", ")}`,
      );
      return { valid: false, errors };
    }
    ...
    if (pluginConfig.type === "agent" && pluginConfig.name) {
      if (BUILTIN_AGENT_NAMES.includes(pluginConfig.name)) {
        errors.push(
          `Name "${pluginConfig.name}" conflicts with a built-in agent type. Custom agents must use unique names.`,
        );
      }
    }
      ...
      for (const [key, propSchema] of Object.entries(schema.properties)) {
        ...
        if (propSchema.maxLength !== undefined && typeof value === "string") {
          if (value.length > propSchema.maxLength) {
            errors.push(
              `Field "${key}" exceeds maximum length of ${propSchema.maxLength} (got ${value.length})`,
            );
          }
        }
  ...
  _securityChecks(config) {
    ...
    for (const field of commandFields) {
      if (typeof config[field] === "string") {
        if (SHELL_INJECTION_PATTERN.test(config[field])) {
          errors.push(
            `Security: field "${field}" contains potentially dangerous shell metacharacters (|, ;, &, $(), backticks). Use simple commands only.`,
          );
        }
      }
    }
    ...
    if (typeof config.payload_template === "string") {
      ...
      if (disallowed && disallowed.length > 0) {
        errors.push(
          `Security: payload_template contains disallowed template variables: ${disallowed.join(", ")}. Only {{event.*}} patterns are allowed.`,
        );
      }
    }
