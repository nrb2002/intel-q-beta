...
class TeamsAdapter extends IntegrationAdapter {
  constructor(options) {
    ...
    this._callbackUrl =
      (options && options.callbackUrl) || process.env.LOKI_TEAMS_CALLBACK_URL || "";
    this._webhookSecret =
      (options && options.webhookSecret) || process.env.LOKI_TEAMS_WEBHOOK_SECRET || "";
  }
  ...
  async createSubtasks(externalId, tasks) {
    return this.withRetry("createSubtasks", async () => {
      ...
      return tasks.map(function (t) {
        return { id: t.title, status: "posted" };
      });
    });
  }
  ...
  getWebhookHandler() {
    ...
    return function (req, res) {
      ...
      req.on("end", function () {
        ...
        var expected = crypto.createHmac("sha256", self._webhookSecret).update(body).digest("hex");
